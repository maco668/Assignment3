...
for i in dim:
    reduced_X = FastICA(n_components=i, random_state=42).fit_transform(X_train_scaled)
    k=30
...
for i in ['km', 'gmm']:
    ...
    if i=='km':
        plt.title('Score vs. dimensionality for K-means with ICA (coverType)')
    else:
        plt.title('Score vs. dimensionality for Gaussian Mixture with ICA (coverType)')
    plt.grid(True)
    ...
    plt.legend()
    plt.savefig(i+"_ICA_Scores_coverType.png")
