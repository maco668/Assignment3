...
for i in dim:
    reduced_X = SparseRandomProjection(n_components=i,random_state=42).fit_transform(X_scaled)
    k=10
...
for i in ['km', 'gmm']:
    ...
    if i=='km':
        plt.title('Score vs. dimensionality for K-means with randomized projection (label=species)')
    else:
        plt.title('Score vs. dimensionality for Gaussian Mixture with randomized projection (label=species)')
    plt.grid(True)
    ...
    plt.legend()
    plt.savefig(i+"_RP_Scores_species.png")
